...
#Point1, point2
# Controls the lines on the escalator for which people have to cross
limitsUp = [50, 300, 400, 300]
limitsDown = [350, 100, 550, 100]

# C
# limitsUp = [100, 400, 400, 300]
# limitsDown = [450, 350, 550, 100]
...
while True:
    ...
    # print(f"limitsDown {limitsDown}")
    # Drawing the horizontal lines for which people cross on the escalator
    ...
    for result in resultsTracker:
        ...
        x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)
        print(result)
        cvzone.cornerRect(img, (x1, y1, x2-x1, y2-y1), l=5, rt=2, colorR=(255,0,255)) #Box around person
        # cvzone.putTextRect(img, f'{currentClass} {id}', (max(0,x1), max(35,y1)), scale=2, thickness=3, offset=10) #Define the class and give the person an ID
        w, h = x2-x1, y2-y1
